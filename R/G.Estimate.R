G.Estimate<-function(Obs.N.all,All.Num.Dist,ML.curve=TRUE,ploidy=1,
  ...
  Obs.Freq<-Obs.N.all
  LociExpFreq<-rowMeans(All.Num.Dist[gmax,,])
  LociExpRelFreq<-rowMeans(All.Num.Dist[dim(All.Num.Dist)[1],,])/sum(LociExpFreq)
  

  Exp.Freq<-sum(Obs.Freq)*LociExpRelFreq
  ChiSq.by.Loci<-(((Obs.Freq-Exp.Freq)^2)/Exp.Freq)
  ChiSq.Stat<-sum(ChiSq.by.Loci)
  ChiSq.Pvalue<-pchisq(ChiSq.Stat,df=nloci-2,lower.tail=F)
  
  Amp.Qual<-t(matrix( c( ChiSq.by.Loci,ChiSq.Stat,ChiSq.Pvalue)))
  colnames(Amp.Qual)<-c(paste("L",1:nloci,sep="."),"ChiSq","P.value")
  
  
  return(list(estimateM=estimateM,LogML=LogML,Amp.Qual=Amp.Qual))
}
